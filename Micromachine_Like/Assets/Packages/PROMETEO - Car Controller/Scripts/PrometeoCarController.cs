...
public class PrometeoCarController : MonoBehaviour
{
    ...
    //[Space(20)]
    [Header("CAR SETUP")] [Space(10)] [Range(20, 500)]
    public int maxSpeed = 500; //The maximum speed that the car can reach in km/h.
    ...
    */
    ...
    void Update()
    {
        ...
        //CAR PHYSIC
        /*The next part is regarding to the car controller. First, it checks if the user wants to use touch controls (for mobile devices) or analog input controls (WASD + Space)
        ...
        if (useTouchControls && touchControlsSetup)
            if (_input.IsMovingForward > Mathf.Epsilon)
            {
        ...
        if (_input.IsMovingBackward > Mathf.Epsilon)
        {
            CancelInvoke("DecelerateCar");
            deceleratingCar = false;
            GoReverse();
        ...
        if (_input.IsTurningLeft > Mathf.Epsilon)
        {
            TurnLeft();
        }

        if (_input.IsTurningRight > Mathf.Epsilon)
        {
            TurnRight();
        }
        ...
        if ((_input.IsMovingForward < Mathf.Epsilon && _input.IsMovingBackward < Mathf.Epsilon))
        {
            ThrottleOff();
        }

        if ((_input.IsMovingForward < Mathf.Epsilon && _input.IsMovingBackward < Mathf.Epsilon) && !handbrakePTI.buttonPressed && !deceleratingCar)
        {
            InvokeRepeating("DecelerateCar", 0f, 0.1f);
            deceleratingCar = true;
        }

        if (_input.IsTurningLeft < Mathf.Epsilon && _input.IsTurningRight < Mathf.Epsilon && steeringAxis != 0f)
        {
            ResetSteeringAngle();
        }
